...
TLV* readTlvFromPipe(int pipeReadEnd){
    ...
    len = read(pipeReadEnd, (void*)tlv , sizeof(tlv->type) + sizeof(tlv->length));
    if (len < 0 || len > sizeof(tlv->value)) { perror("type + length"); delete(tlv); return nullptr; }
...
void writeTlvToPipe(int pipeWriteEnd, TLV* tlv){
    ...
    if (len < 0) { perror("type + length + value"); return; }
}
